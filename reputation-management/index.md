...
Review requests are triggered automatically based on events in the platform. After a completed appointment, a closed pipeline opportunity, or a defined workflow trigger, the system sends a review request through the configured channel without manual initiation.
...
The platform aggregates incoming reviews from connected platforms into a single dashboard. Agencies monitor review activity across all client accounts without logging into each review platform individually.

- View incoming reviews from Google and Facebook in one interface
- Receive notifications when new reviews are posted
- Track review volume, average rating, and rating trend over time
- Identify accounts with declining review velocity or rating drops
...
- Assign review response tasks to specific team members
- Log all response activity at the sub-account level
